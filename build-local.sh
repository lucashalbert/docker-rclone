...
for docker_arch in amd64 arm32v6 arm64v8; do
    case ${docker_arch} in
        amd64   ) qemu_arch="x86_64"  rclone_arch="amd64" s6_arch="amd64"   ;;
        arm32v6 ) qemu_arch="arm"     rclone_arch="arm"   s6_arch="arm"     ;;
        arm64v8 ) qemu_arch="aarch64" rclone_arch="arm64" s6_arch="aarch64" ;;    
    esac
    ...
    if [ "$EUID" -ne 0 ]; then
        ...
        docker build -f Dockerfile.${docker_arch} -t lucashalbert/rclone:${docker_arch}-${rclone_ver}-s6-beta .
